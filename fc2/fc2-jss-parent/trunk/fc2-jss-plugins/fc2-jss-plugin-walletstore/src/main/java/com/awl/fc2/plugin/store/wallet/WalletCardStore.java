...
public class WalletCardStore implements ICardStoreV2 {
	static Logger log = new Logger(WalletCardStore.class);
	public static void trace(Object msg){
		log.trace(msg);
	}
	WalletCardStore_OldVersion store = new WalletCardStore_OldVersion();
	...
	public void addInfoCard(InfoCard cardToAdd, boolean onlyMemory) {
		if(onlyMemory){
			...
			String xml;
			try {
				if(cardToAdd instanceof SignedInfoCard){
					xml = ((SignedInfoCard) cardToAdd).toXML();
				}else{
					xml = cardToAdd.toXML();
				}
...
//			}
	...
	public ICredentialsStore getCredentialStore() {
		// TODO Auto-generated method stub
		if(creds==null) creds = new CredentialStore();
		return creds;
	}
