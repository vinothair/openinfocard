...
import com.awl.rd.fc2.data.connectors.services.telcos.TelcosData;
import com.utils.Base64;
import com.utils.ISTSConfiguration;

public class STSConfiguration_Telcos implements ISTSConfiguration {

	static Logger log = Logger.getLogger(STSConfiguration_Telcos.class);
	...
	public void createAccountForUserId(String userid,String pwd) throws CryptoException{
		trace("Creating User Telcos Account - "+userid);
		connect.addUser(userid,pwd);		
		CompositeSupportedClaims theClaims = new CompositeSupportedClaims();	
		theClaims.addSupportedClaims(new TelcosSupportedClaims());		
		connect.addNewCardToTheUser(userid,getCardIDFromUserId(userid),theClaims);			
		connect.configureService(userid, ServiceType.TELCOS, userid);
		DBUtils.getInstance().addUser(userid, TelcosData.getDefaultUser(userid));
