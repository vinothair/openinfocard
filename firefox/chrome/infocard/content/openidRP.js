var StreamListener = function(aChannel, anOpenIdRpInstance) {
...
StreamListener.prototype = {
	...
	getInterface : function(aIID) {
		dump('openidRP QueryInterface: aIID=' + aIID);
		try {
			return this.QueryInterface(aIID);
		} catch (e) {
			dump('openidRP QueryInterface: exception=' + e);
...
//	  mOpenIdRpInstance.logService.logStringMessage('openidRP StreamListener: onRedirect: aOldChannel.URI=' + aOldChannel.URI);
...
openidRP.prototype = {
	...
	updateProgress : function(evt) {
		try {
			var label = this.aDoc.getElementById("notify");
	...
	onStateChange : function(aProgress, aRequest, aFlag, aStatus) {
		...
		IdentitySelectorDiag.logMessage("openidRP.onStateChange", "flags = "
				+ aFlag);

		if (aFlag & progListIFace.STATE_IS_DOCUMENT) {
			IdentitySelectorDiag.logMessage("openidRP.onStateChange",
					"flag & document");
		}
	...
	onLocationChange : function(aProgress, aRequest, aURI) {
		...
		try {
			IdentitySelectorDiag.debugReportError(
					"openidRP.onLocationChange location=",
					aProgress.DOMWindow.document.location);
		} catch (e) {
			IdentitySelectorDiag.debugReportError("openidRP.onLocationChange",
					e);
		}
	...
	checkimmediateCallback : function(status, data) {
		var logService = Components.classes["@mozilla.org/consoleservice;1"]
				.getService(Components.interfaces.nsIConsoleService);
		logService.logStringMessage("checkimmediateCallback Status=" + status);
		if (data !== null) {
    ...
    anUri += extraParams;
		...
		this.logService
				.logStringMessage("checkimmediateCallback: user_setup_url="
						+ user_setup_url);
...
//		window.stop(); // FIXME
//		var view = this.aDoc.defaultView;
//		if (view) {
//		  view.stop();
//		}
    var iframe = this.aDoc.getElementById('openid_iframe');
    if (iframe) {
      iframe.stop(3); // 3 = stopall https://developer.mozilla.org/en/nsIWebNavigation#Stop_Flags
    }
	},
	...
	handleDiscoveryResponseHtml : function(resp) {
...
//	</xrds:XRDS>
	...
	handleRetrievedXrds : function(xrds) {
		try {
			var sortedServices = [];
			var label = this.aDoc.getElementById("notify");
			if (label) {
			...
			this.logService.logStringMessage("handleRetrievedXrds extraParamsOpenIDAuthParameters=" + 
					this.extraParamsOpenIDAuthParameters );
			var services = xrds.getElementsByTagNameNS("xri://$xrd*($v*2.0)", "Service");
		...
		try {
			...
			for (var i=0; i<sortedServices.length; i++) {
				...
				for (var j=0; i<services.length; i++) {
					...
					for (var m=0; m<types.length; m++) {
						...
						this.logService.logStringMessage("retrieveXrds Protocol=\"" + protocol + "\"");
						if ((protocol.length > 0) && (typeStr.indexOf(protocol) === 0)) {
			...
			if (matchingService != null) {
				var id = this._urlEncode(matchingService.localId);
				var extraParams = this._extraParamsToUrl(this.extraParamsOpenIDAuthParameters);
    ...
    req.setRequestHeader('Accept', "application/xrds+xml");  
      ...
      if ((respContentType) && (respContentType.indexOf("application/xrds+xml") >= 0)) {
        ...
        this.handleRetrievedXrds(req.responseXML);
      } else {
        // X-XRDS-Location: https://pip.verisignlabs.com/user/ignisvulpis/yadisxrds
  			...
  			if (xrds_location !== null) {
  				...
  				this.handleRetrievedXrds(xrds);
  			} else {
  				this.handleDiscoveryResponseHtml(resp);
  			}
      }
		} else {
			if ((this.finalizeOpenId !== undefined)
