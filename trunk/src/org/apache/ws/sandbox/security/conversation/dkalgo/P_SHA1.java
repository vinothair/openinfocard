...
public class P_SHA1 {
    ...
    public byte[] createKey(byte[] secret, byte[] labelAndNonce, int offset,long length) throws Exception {
        try {
            Mac mac = Mac.getInstance("HmacSHA1");

            //byte[] tempBytes = P_hash(secret, labelAndNonce.getBytes(), mac,(offset + (int) length));
            byte[] tempBytes = P_hash(secret, labelAndNonce, mac,(offset + (int) length));
            ...
            throw new Exception("Key Derivation : P_SHA-1: " +
                    ex.getMessage());
        }
    }
